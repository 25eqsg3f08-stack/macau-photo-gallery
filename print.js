...
const CONFIG = {
    ...
    MOBILE_MAX_WIDTH: 350, // 移动端图片最大宽度
    DESKTOP_MAX_WIDTH: 800 // 桌面端图片最大宽度
};
...
async function initPrintPage() {
    ...
    const defaultWidth = isMobile ? CONFIG.MOBILE_MAX_WIDTH : CONFIG.DESKTOP_MAX_WIDTH;
    const defaultHeight = Math.round(defaultWidth * 0.625); // 16:10比例

    // 设置默认值
    if (el.printDate) el.printDate.value = new Date().toISOString().split("T")[0];
    if (el.imgWidth) el.imgWidth.value = defaultWidth;
    if (el.imgHeight) el.imgHeight.value = defaultHeight;
...
function handleWindowResize() {
    ...
    const isMobile = window.innerWidth < 768;
    const maxWidth = isMobile ? CONFIG.MOBILE_MAX_WIDTH : CONFIG.DESKTOP_MAX_WIDTH;
    const maxHeight = Math.round(maxWidth * 0.625);
    
    // 仅当当前尺寸超过设备最大限制时调整
    if (parseInt(el.imgWidth.value) > maxWidth) {
        el.imgWidth.value = maxWidth;
        el.imgHeight.value = maxHeight;
        loadCurrentImg(); // 重新加载图片应用新尺寸
    }
}
...
function loadCurrentImg() {
    ...
    // 移动端强制限制图片最大宽度
    const isMobile = window.innerWidth < 768;
    let setWidth = parseInt(el.imgWidth.value);
    let setHeight = parseInt(el.imgHeight.value);
    
    if (isMobile && setWidth > CONFIG.MOBILE_MAX_WIDTH) {
        setWidth = CONFIG.MOBILE_MAX_WIDTH;
        setHeight = Math.round(setWidth * (setHeight / parseInt(el.imgWidth.value)));
    }

    // 设置图片属性（添加max-width保证不溢出）
    el.printImg.src = imgUrl;
    el.printImg.style.width = `${setWidth}px`;
    el.printImg.style.height = `${setHeight}px`;
    el.printImg.style.maxWidth = "100%"; // 关键：防止图片超出容器
    el.printImg.style.height = "auto"; // 自动高度保持比例
...
function applyConfig() {
    ...
    if (el.printImg && el.imgWidth && el.imgHeight) {
        ...
        el.printImg.style.width = `${setWidth}px`;
        el.printImg.style.height = "auto";
        el.printImg.style.maxWidth = "100%";
    }
}
