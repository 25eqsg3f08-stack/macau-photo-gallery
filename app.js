...
async function getImgListWithRetry(retryLeft) {
    ...
    // 新增：前置校验仓库是否存在/可访问
    try {
        const repoCheckRes = await fetch(`https://api.github.com/repos/${REPO.USER}/${REPO.REPO}`, {
            headers: { 
                "User-Agent": "MacauGallery/1.0", // GitHub API强制要求User-Agent
                "Accept": "application/vnd.github.v3+json"
            },
            cache: "no-store"
        });
        if (repoCheckRes.status === 404) throw new Error("目标仓库不存在，请确认仓库名是否正确");
        if (repoCheckRes.status === 403) throw new Error("仓库为私有，无法公开访问图片");
        if (!repoCheckRes.ok) throw new Error(`仓库校验失败 [${repoCheckRes.status}]`);
    } catch (repoErr) {
        throw new Error(`仓库验证错误：${repoErr.message}`);
    }
    ...
    try {
        const res = await fetch(REPO.API, {
            headers: {
                "User-Agent": "MacauGallery/1.0", // 避免GitHub API限流
                "Accept": "application/vnd.github.v3+json"
            },
            cache: "no-store",
            mode: "cors"
        ...
        if (res.status === 404) throw new Error("图片仓库目录不存在");
        ...
        const newImgList = files.filter(file => {
            ...
            // 修复：对文件名做URL编码，处理空格/特殊字符
            const encodedFileName = encodeURIComponent(file.name);
            return REPO.RAW + encodedFileName;
        });
...
function cacheImg(url) {
    ...
    // 修复：添加请求头确保GitHub Raw资源正常缓存
    navigator.serviceWorker.controller.postMessage({ 
        type: "CACHE_IMG", 
        url,
        headers: { 
            "Accept": "image/*",
            "User-Agent": "MacauGallery/1.0"
        } 
    });
